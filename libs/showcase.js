...
views.categories = new stick.View({
  ...
  select: function (name, target) {
    ...
    var previous = this.current.el || root.find('div:nth(1)');
...
views.showcase = new stick.View({
  ...
  render: function (category) {
    var root = this.root;
    var elements = this.elements;
    var container = this.elements.container = $('<div class="panel">');
    var name = category;
...
views.detail = new stick.View({
  ...
  template: doT.template($('#template-detail').html()),
  ...
  render: function (demo) {
    ...
    $.ajax({
      ...
      success: function (data) {
        container = elements.container = $(template(params));
        ...
        var code = container.find('.detail-code')[0];
  ...
  resize: function () {
    if (this.displayed) {
      ...
      var computed_height = $('body').height() - pageHeaderHeight - detailHeaderHeight - 20;
      container.height(computed_height);
