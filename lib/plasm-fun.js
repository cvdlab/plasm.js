 ...
 */
 ...
 !(function (exports) {
   ...
   * SHOW
   *
   * @param {plasm.Model} model to show
   * @return {plasm.Mode} model
   * @api public
   */

  var SHOW = 
  fun.SHOW = function (model) {
    model.show();
  };
  ...
  fun.SIMPLICIALCOMPLEX = function (points) {
    return function (complex) {
      return p.simplicialComplex(points, complex);
    };
  };
  ...
  var SIMPLEXGRID = 
  fun.SIMPLEXGRID = function (quotes) {
    return p.simplexGrid(points);
  };
