 ...
 */
 ...
 !(function (exports) {
  ...
  plasm.Viewer = function (containerEl, statsEl) {
  ...
  plasm.Viewer.prototype.draw = function (obj) {
    ...
    if (dim === 1) {
      ...
      for (var i = 0; i < cells.length; i += 2) {
        var v1 = pointset.get(cells[i + 0]);
        var v2 = pointset.get(cells[i + 1]);
    ...
    if (dim >= 2) {
      ...
      for (var i = 0; i < cells.length; i += 3) {
        geometry.faces.push(new THREE.Face3(cells[i+0], cells[i+1], cells[i+2]));
      };
  ...
  plasm.Viewer.prototype.simplicialComplex = function (points, complex) {
    return new plasm.SimplicialComplex(this, points, complex);
  };

  plasm.SimplicialComplex = function (viewer, points, complex) {
    if (!(this instanceof plasm.SimplicialComplex)) {
      return new plasm.SimplicialComplex(viewer, points, complex);
    }

    this.sc = new simplexn.SimplicialComplex(points, complex);
    this.model = viewer.draw(this.sc);
    this.model.hide();
  };

  plasm.SimplicialComplex.prototype.show = function () {
    this.model.show();
    return this;
  };

  plasm.SimplicialComplex.prototype.hide = function () {
    this.model.hide();
    return this;
  };

  plasm.SimplicialComplex.prototype.translate = function (dims, values) {
    var model = this.model;
    this.sc.translate(dims, values);
    this.model = viewer.draw(this.sc);
    model.hide();
  };
