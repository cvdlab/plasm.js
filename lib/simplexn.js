(function() {
  var AA, AL, APPLY, AR, BIGGER, BIGGEST, BOUNDARY, BUTLAST, CART2, CAT, CENTROID, CIRCLE, CLONE, CODE, COMP, CONS, CUBE, CUBOID, CYLSOLID, CYLSURFACE, DISK, DISTL, DISTR, DIV, E, EMBED, EXPLODE, EXTRUDE, F1, FREE, GRAPH, HELIX, ID, IDNT, INSL, INSR, INTERVALS, ISFUN, ISNUM, K, LEN, LINSPACE1D, LINSPACE2D, LINSPACE3D, LIST, MAP, MAT, MUL, PI, POLYLINE, PRECISION, PRINT, PROGRESSIVE_SUM, PointSet, QUADMESH, R, REPEAT, REPLICA, REVERSE, S, SET, SIMPLEX, SIMPLEXGRID, SKELETON, SMALLER, SMALLEST, SUB, SUM, SimplicialComplex, T, TAIL, TORUSSOLID, TORUSSURFACE, TRANS, TREE, TRIANGLEARRAY, TRIANGLEFAN, TRIANGLESTRIP, Topology, VECT, abs, acos, asin, atan, atan2, ceil, cos, decode, exp, fcode, fixedPrecision, floor, log, root, round, sin, sqrt, string2numberList, tan, type, typedPrint, uncode;
  ...
  root.TREE = TREE = function(f) {
    var uncurriedTree;
    uncurriedTree = function(fun, array) {
      var k, len;
      len = array.length;
      if (len === 1) return array[0];
      k = floor(len / 2);
      return fun(CAT([uncurriedTree(fun, array.slice(0, k)), uncurriedTree(fun, array.slice(k, len))]));
    };
    return function(array) {
      return uncurriedTree(f, array);
    };
  };
