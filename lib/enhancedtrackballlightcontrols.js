THREE.EnhancedTrackballLightControls = function ( object, scene, domElement ) {
  ...
  var _this = this
    , scene
    , STATE = { NONE : -1, ROTATE : 0, ZOOM : 1, PAN : 2, RESET: 3 }
    , keys = {
        r: 82
      , c: 67
      , bar: 32
  ...
  this.update = function() {
    ...
    if ( _state === STATE.RESET ) {
      ...
      // console.log('--------------------camera:\n\tx:', _this.object.position.x,'y:', _this.object.position.y,'z:', _this.object.position.z, _this.object.matrix );
      // console.log('--------------------scene:\n\tx:', light.position.x,'y:', light.position.y,'z:', light.position.z, light.matrix );

      var diff = new THREE.Vector3();
      diff.sub(_this.object.position, light.position);
      if (diff.x !== 0 || diff.y !== 0 || diff.z != 0) { console.log(diff); }

      var mdiff = new THREE.Matrix4();
      mdiff = _this.object.matrix.determinant() - light.matrix.determinant();
      if (mdiff !== 0) {console.log(_this.object.matrix, light.matrix)}
    }
  };
  ...
  this.placeCamera = function(vector, target, up) {
      ...
      light.position = vector || light.position;
