 ...
 */
 ...
 !(function (exports) {
  ...
  plasm.Viewer = function (container) {
    ...
    var controls = this.controls = new plasm.Controls(camera);
    ...
    light = new THREE.PointLight(0xaa9988);
    light.position.set(-100, -100, -100);
    scene.root.add(light);

    light = new THREE.DirectionalLight(0xaa9988);
  ...
  plasm.Controls = function (camera, options) {
    if (!(this instanceof plasm.Controls)) {
      return new plasm.Controls(camera, options);
    }
    ...
    controls = new THREE.TrackballControls(camera.optics);
    ...
    controls.staticMoving = options.staticMoving || true;
    controls.dynamicDampingFactor = options.dynamicDampingFactor || 0.3;
    controls.keys = options.keys || [65, 83, 68]; // keys A, S, D
