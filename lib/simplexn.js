(function() {
  var AA, AL, APPLY, AR, BIGGER, BIGGEST, BOUNDARY, BUTLAST, CART2, CAT, CENTROID, CIRCLE, CLONE, CODE, COMP, CONS, CUBE, CUBOID, CYLSOLID, CYLSURFACE, DISK, DISTL, DISTR, DIV, E, EMBED, EXPLODE, EXTRUDE, F1, FREE, GRAPH, HELIX, ID, IDNT, INSL, INSR, INTERVALS, ISFUN, ISNUM, K, LEN, LINSPACE2D, LINSPACE3D, LIST, MAP, MAT, MUL, MYPRINT, PI, POLYLINE, PRECISION, PROGRESSIVE_SUM, PointSet, QUADMESH, R, REPEAT, REPLICA, REVERSE, S, SET, SIMPLEX, SIMPLEXGRID, SKELETON, SMALLER, SMALLEST, SUB, SUM, SimplicialComplex, T, TAIL, TORUSSOLID, TORUSSURFACE, TRANS, TREE, TRIANGLEFAN, TRIANGLE_ARRAY, TRIANGLE_STRIP, Topology, VECT, abs, acos, asin, atan, atan2, ceil, cos, decode, exp, fcode, fixedPrecision, floor, log, root, round, sin, sqrt, string2numberList, tan, type, typedPrint, uncode;
  ...
  root.TREE = TREE = function(f) {
    var uncurriedTree;
    uncurriedTree = function(fun, List) {
      var k, len;
      len = List.length;
      if (len === 1) return List[0];
      k = floor(len / 2);
      return fun(CAT([uncurriedTree(fun, List.slice(0, k)), uncurriedTree(fun, List.slice(k, len))]));
    };
    return function(x) {
      return uncurriedTree(f, x);
    };
  };
  ...
  */
  ...
  root.LINSPACE2D = LINSPACE2D = function(sizes) {
    return function(subsets) {
      var a, b, m, n, pointMat;
      MYPRINT("sizes =", sizes);
      MYPRINT("subsets =", subsets);
      a = sizes[0], b = sizes[1];
      n = subsets[0], m = subsets[1];
  ...
  root.LINSPACE3D = LINSPACE3D = function(sizes) {
    return function(subsets) {
      var a, b, c, domain2d, hLists, m, n, p;
      a = sizes[0], b = sizes[1], c = sizes[2];
      n = subsets[0], m = subsets[1], p = subsets[2];
