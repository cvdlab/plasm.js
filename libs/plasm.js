 ...
 */
 ...
 !(function (exports) {
  ...
  plasm.Viewer = function (container, inspector) {
    ...
    var light = this.light = new THREE.AmbientLight(0x222222);
    scene.root.add(light);

    var axes = this.axes();
    scene.root.add(axes.models[0].mesh);
    scene.root.add(axes.models[1].mesh);
    scene.root.add(axes.models[2].mesh);
  ...
  plasm.Scene = function () {
    ...
    this.obj_root = new THREE.Object3D();
    this.root.add(this.obj_root);
    this.models = [];
  };
  ...
  plasm.Scene.prototype.add = function (object) {
    ...
    if (object instanceof plasm.Model) {
      this.root.children[0].add(object.mesh);
  ...
  plasm.Scene.prototype.remove = function (object) {
    ...
    if (object instanceof plasm.Model) {
      this.root.children[0].remove(object.mesh);
      this.models.splice(this.models.indexOf(object), 1);
      return this;
    }
    return this;
  };
   ...
   * remove all the drawn models from the scene
   * and erase them. 
   *
   * @return {plasm.Scene} for chaining
   * @api public
  ...
  plasm.Scene.prototype.reset = function () {
  ...
  plasm.Scene.prototype.getBoundingSphere = function () {
    var radius = 0;
    var bbox = new THREE.Box3().setFromObject(this.obj_root);
  ...
  plasm.Scene.prototype.getCentroid = function () {
    ...
    return centroid.addVectors(bbox.max, bbox.min).divideScalar(2);
  };
  ...
  plasm.Controls = function (camera, scene, elements, render) {
    if (!(this instanceof plasm.Controls)) {
      return new plasm.Controls(camera, scene, elements, render);
    }

    var options = options || {};
    var controls;

    controls = new THREE.TrackballControls(camera.optics, scene, elements);

    controls.rotateSpeed = options.rotateSpeed || 3.0;
  ...
  plasm.Model.prototype.translate = function (dims, values) {
    ...
    if (this.complex.rn <= 3) {
      ...
      this.mesh.position.add({ x: v[0] || 0, y: v[1] || 0, z: v[2] || 0 });
  ...
  plasm.Model.prototype.rotate = function (dims, angle) {
    ...
    if (this.complex.rn <= 3) {
      ...
      this.mesh.rotation.fromArray([v[0] || 0, v[1] || 0, v[2] || 0 ]);
      this.geometry.__dirtyVertices = true;
    }
  ...
  plasm.Model.prototype.scale = function (dims, values) {
    ...
    if (this.complex.rn <= 3) {
      ...
      this.mesh.scale.multiply({ x: v[0] || 1, y: v[1] || 1, z: v[2] || 1 });
      this.geometry.__dirtyVertices = true;
    }
